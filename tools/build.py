...
files = list((pack / 'assets').rglob('*'))
files.extend(pack / name for name in ('pack.mcmeta', 'pack.png', 'assets.ajmeta'))
...
with ZipFile(output) as archive:
    ...
    for file in files:
        assert archive.read(file.relative_to(pack).as_posix()) == file.read_bytes()
