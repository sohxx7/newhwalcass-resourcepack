...
리소스팩 작업의 commit/pull/push는 두 번째 경로에서 실행합니다. 서버 저장소의 기존 `/pack` 제외 설정은 유지합니다. 문서·편집 자료·도구·`.git`은 `pack/` 밖에 두어 `/build`와 릴리스 ZIP에 들어가지 않게 합니다. 새 환경에서 연결을 만들 때에는 기존 서버 `pack`을 먼저 백업하고 양쪽 파일을 비교·병합해야 합니다. 변경이 남아 있는 상태에서 강제 pull/reset이나 폴더 덮어쓰기를 하지 않습니다.
